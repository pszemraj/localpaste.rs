...
pub(crate) fn copy_dir_recursive(src: &Path, dst: &Path) -> Result<(), AppError> {
    fs::create_dir_all(dst).map_err(|err| {
        AppError::StorageMessage(format!("Failed to create backup directory: {}", err))
    })?;

    for entry in fs::read_dir(src)
        .map_err(|err| AppError::StorageMessage(format!("Failed to read directory: {}", err)))?
    {
        let entry = entry.map_err(|err| {
            AppError::StorageMessage(format!("Failed to read directory entry: {}", err))
        })?;
        ...
        if path.is_dir() {
            ...
            fs::copy(&path, &dst_path).map_err(|err| {
                AppError::StorageMessage(format!("Failed to copy file {:?}: {}", path, err))
            })?;
        }
    }

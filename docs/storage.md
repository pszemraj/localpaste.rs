...
GUI delete undo is intentionally small and bounded. It moves the paste row and its version rows into `deleted_*` staging tables inside the same write transaction that removes the active rows and updates folder projections. Undo restore moves those rows back into the active tables by token. Expired or overflowed undo tokens are discarded by the GUI backend worker, and expired staged rows are also pruned on database startup before normal server, CLI-through-server, and backup paths run. Backups may contain currently live delete-undo staging rows, but expired rows are not kept as an old undo window. This staging area is not durable history, redo state, or a retention policy; version history is the recovery surface after the short delete-undo window closes.

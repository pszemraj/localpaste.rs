...
- [ ] Revisit the remaining `check-ast-dupes --include-tests` near-miss pairs only if a measured cleanup reduces LOC or clarifies behavior: `localpaste_server/tests/manual_language_create.rs` covers different language-state transitions, and `localpaste_gui/src/app/tests/keyboard_navigation_audit.rs` covers distinct cursor semantics.

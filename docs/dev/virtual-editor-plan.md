...
- Add/confirm drag auto-scroll behavior when selecting beyond viewport edges.
- Performance gate sign-off in release mode:
  - average FPS `>= 45`
  - p95 frame time `<= 25 ms`
  - no visible hitching during rapid scroll + mid-document typing on the 5k-line scenario

...
Every Windows run also writes a manifest next to the NDJSON log by default (`*.manifest.json`) with the selected scenarios, repeat labels, input driver, log path, and final assertion status. Aborted runs finalize the manifest with `status: "failed"` and an `error` field before rethrowing. Use `-Manifest <path>` to override it.

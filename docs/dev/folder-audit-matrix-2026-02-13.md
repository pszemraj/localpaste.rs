...
| `DELETE /api/folder/:id` | folder tree deletes + paste migrations | paste meta/index reconcile + count movement | locked descendant precheck; delete-marker set | `server::handlers::folder`, `core::folder_ops::delete_folder_tree_and_migrate` | reject on lock conflict; no orphan target refs | `test_delete_folder_rejects_when_descendant_paste_is_locked`, `test_folder_lifecycle` | Covered |
...
| GUI `CoreCmd::DeleteFolder` | delete-tree + migration | index reconcile + count movement | locked descendant precheck | GUI backend uses `core::first_locked_paste_in_folder_delete_set` | reject on locked descendant | `locked_descendant_blocks_backend_folder_delete` | Covered |
